...
V2_ACCURACY_WEIGHT = 0.70
V2_EXECUTABLE_WEIGHT = 0.08
V2_FORMAT_WEIGHT = 0.02

V2_LIST_TABLES_WEIGHT = 0.03
V2_SCHEMA_WEIGHT = 0.05
V2_EXECUTE_TOOL_WEIGHT = 0.08
V2_RECOVERY_WEIGHT = 0.02
V2_ORDERED_CHAIN_WEIGHT = 0.02
...
def compute_score(
    ...
    tool_metrics = extract_tool_process_metrics(
        solution_str
    )
    ...
    list_tables_success = float(
        tool_metrics["list_tables_succeeded"] > 0.5
    )
    schema_success = float(
        tool_metrics["get_table_schema_succeeded"] > 0.5
    )
    execute_after_schema_success = float(
        tool_metrics[
    ...
    ordered_chain = float(
        tool_metrics["ordered_tool_chain"] > 0.5
    )

    accuracy_reward = (
        V2_ACCURACY_WEIGHT * execution_correct
    )
    executable_reward = (
        V2_EXECUTABLE_WEIGHT * sql_executable
    )
    format_reward = (
        V2_FORMAT_WEIGHT * format_compliance
    )

    list_tables_reward = (
        V2_LIST_TABLES_WEIGHT * list_tables_success
    )
    schema_reward = (
        V2_SCHEMA_WEIGHT * schema_success
    )
    execute_tool_reward = (
        V2_EXECUTE_TOOL_WEIGHT
        * execute_after_schema_success
    )
    recovery_reward = (
        V2_RECOVERY_WEIGHT * recovery_success
    )
    ordered_chain_reward = (
        V2_ORDERED_CHAIN_WEIGHT * ordered_chain
    )
    ...
    score = terminal_reward + process_reward
    ...
    return {
        ...
        "list_tables_reward": list_tables_reward,
        "schema_reward": schema_reward,
        "execute_tool_reward": execute_tool_reward,
        "recovery_reward": recovery_reward,
        "ordered_chain_reward": ordered_chain_reward,
